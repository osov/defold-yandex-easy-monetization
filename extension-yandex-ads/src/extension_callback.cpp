...
namespace dmYandexAds {
	...
	static void InvokeCallback(MessageId type, const char *json) {
		if (!dmScript::IsCallbackValid(m_luaCallback)) {
			dmLogError("ExtensionYandexAds callback is invalid. Set new callback unsing `yandexads.setCallback()` function.");
			return;
		}
		...
		dmScript::PCall(L, number_of_arguments, 0);
	...
	void AddToQueueCallback(MessageId type, const char *json) {
		CallbackData data;
		data.msg = type;
		data.json = json ? strdup(json) : nullptr;
